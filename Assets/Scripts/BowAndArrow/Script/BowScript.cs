    ...
    public class BowScript : MonoBehaviour
    {
        ...
        private void ContinueBowDraw()
        {
            ...
            if (!AllowFizzling || DrawTotalArea.OverlapPoint(Camera.main.ScreenToWorldPoint(Input.mousePosition)))
            {
                ...
                currentArrow.GetComponent<Rigidbody>().MovePosition(pos);
                currentArrow.GetComponent<Rigidbody>().MoveRotation(BowShaft.transform.rotation.normalized);
                RenderBowString(pos);
            }
            else
            {
        ...
        private void FireArrow(bool fizzle)
        {
            ...
            currentArrow.GetComponent<Rigidbody>().velocity =
                currentArrow.transform.GetChild(0).gameObject.GetComponent<Rigidbody>().velocity = BowShaft.transform.rotation * new Vector3(-speed, 0.0f);
